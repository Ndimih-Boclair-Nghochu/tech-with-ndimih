...
class Product(models.Model):
    ...
    live_url = models.URLField(max_length=500, blank=True, help_text='URL to view the project live')
    whatsapp_url = models.URLField(max_length=1000, blank=True, null=True, help_text='WhatsApp message link for product inquiries')
    youtube_url = models.URLField(max_length=1000, blank=True, null=True, help_text='Optional YouTube video URL for product demo')
    github_url = models.URLField(max_length=500, blank=True, null=True, help_text='URL to view the project on GitHub')
...
class ProjectForSale(models.Model):
    """Projects available for sale or collaboration"""
    title = models.CharField(max_length=200)
    slug = models.SlugField(max_length=210, unique=True, blank=True)
    description = models.TextField(blank=True)
    image = models.ImageField(upload_to='projects-for-sale/', blank=True, null=True)
    price = models.DecimalField(max_digits=10, decimal_places=2, blank=True, null=True, help_text='Price in USD')
    whatsapp_url = models.URLField(max_length=500, blank=True, help_text='WhatsApp chat URL')
    live_url = models.URLField(max_length=500, blank=True, help_text='Live project URL or YouTube video')
    is_published = models.BooleanField(default=True)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            ...
            slug = base
            i = 1
            while ProjectForSale.objects.filter(slug=slug).exists():
                slug = f"{base}-{i}"
                i += 1
            self.slug = slug
        super().save(*args, **kwargs)
