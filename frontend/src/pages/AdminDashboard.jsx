                ...
                <div className="content-card">
                  ...
                  <form onSubmit={handleCreateProduct}>
                    ...
                    <div className="form-group">
                      ...
                      />
                      {productFormPreview && (
                        <img src={productFormPreview} alt="preview" className="preview-image" />
                      )}
                      {productForm.file && !productFormPreview && (
                        <div className="preview-file">
                          {productForm.file.name} ({(productForm.file.size/1024/1024).toFixed(2)} MB)
                        </div>
                      )}
                    </div>
                    <div className="form-group">
                    ...
                    <div className="btn-group">
                      <button 
                        className="btn btn-success" 
                        type="submit" 
                        disabled={productStatus === 'creating'}
                      >
                        {productStatus === 'creating' && <span className="spinner"></span>}
                        {productStatus === 'creating' ? 'Creating...' : 'Create Product'}
                      </button>
                    </div>
                  </form>
                </div>
                      ...
                      {products.map(prod => (
                        <div key={prod.id} className="item-card">
                          <div className="item-info">
                            ...
                            <div className="item-meta">{prod.description || 'No description'}</div>
                            {prod.affiliate_url && (
                              <div className="item-badge" style={{background: '#dbeafe', color: '#1e40af'}}>
                                Affiliate • {prod.affiliate_clicks || 0} clicks
                              </div>
                            )}
                          </div>
                          <div className="item-actions">
                            ...
                            <button className="btn btn-warning" onClick={()=>openProductEdit(prod)}>
                              Edit
                            </button>
                            {prod.affiliate_url && (
                              <button 
                                className="btn btn-primary" 
                                onClick={()=>window.open(`/api/products/${prod.id}/go/`, '_blank')}
                              >
                                Open
                              </button>
                            )}
                            <button className="btn btn-danger" onClick={()=>handleDeleteProduct(prod.id)}>
                              Delete
                            </button>
                          </div>
                        </div>
          ...
          <div className="modal-overlay" onClick={() => setProductEdit(null)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()}>
              ...
              <form onSubmit={handleUpdateProduct}>
                ...
                <div className="form-group">
                  <label>Affiliate / External URL</label>
                  <input 
                    type="url" 
                    value={productEditAffiliate} 
                    onChange={e=>setProductEditAffiliate(e.target.value)}
                  />
                </div>
                <div className="form-group">
